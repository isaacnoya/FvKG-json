        ...
        rows.append(
            {
                ...
                "fvkg_json": len(set(getMappingsFromBGP(MappingContext(), tps, mappings))),
            }
        )
...
def aux():
    ...
    mappings = getMappingsFromFolder("/Users/kekojohns/Library/CloudStorage/OneDrive-Personal/muia/oeg/tfm/eval/initialMappingTemplates/mappings")

    #q = prepareQuery(Path("/Users/kekojohns/Desktop/pruningOlaf/resources/queries/q18.rq").read_text(encoding="utf-8"))
    q = prepareQuery(Path("/Users/kekojohns/Library/CloudStorage/OneDrive-Personal/muia/oeg/tfm/eval/vkg/queries/q02.rq").read_text(encoding="utf-8"))
    tps = triples_from_algebra(q.algebra)

    ms_fvkg_json = set(getMappingsFromBGP(MappingContext(), tps, mappings))
    ms_itt = mappingCandidateSelectionITT(mappings, tps)

    print(f"Len FvKG-json {len(list(ms_fvkg_json))}, Len ITT {len(ms_itt)}")

    ms_itt = set(ms_itt)
    print(f"In ms_fvkg_json and not in ms_itt: {len(ms_fvkg_json - ms_itt)}")
    for m in ms_fvkg_json - ms_itt:
        print(m.s, m.p, m.o, m.source)
    print(f"In ms_itt and not in ms_fvkg_json: {len(ms_itt - ms_fvkg_json)}")
    for m in ms_itt - ms_fvkg_json:
        print(m.s, m.p, m.o, m.source)
