...
export function ExecutionConsole({
  ...
  return (
    <Collapsible.Root
      className={cn(
        "absolute bottom-4 right-4 z-30 overflow-hidden rounded-xl border border-border shadow-console backdrop-blur-xl",
        theme === "light" ? "bg-white/95" : "bg-[#070b11]/95",
      )}
    ...
    >
      <div className="flex h-11 items-center justify-between border-b border-border px-3.5">
        ...
        <div className="flex items-center gap-1">
          <div className="mr-1 flex items-center rounded-md border border-border bg-secondary/70 p-0.5">
            <button
              className={cn(
                "rounded px-2 py-1 text-[9px] uppercase tracking-wider text-muted-foreground transition-colors",
                view === "logs" && "bg-background text-foreground shadow-sm",
              )}
            ...
            <button
              className={cn(
                "rounded px-2 py-1 text-[9px] uppercase tracking-wider text-muted-foreground transition-colors",
                view === "results" && "bg-background text-primary shadow-sm",
              )}
      ...
      <Collapsible.Content>
            ...
            {!results || results.variables.length === 0 ? (
              ...
              <table className="min-w-full border-collapse font-mono text-[10px]">
                <thead className="sticky top-0 z-10 bg-secondary">
                  <tr>
                    <th className="border-b border-r border-border px-3 py-2 text-right font-normal text-muted-foreground">
                      #
                    </th>
                    {results.variables.map((variable) => (
                      <th
                        className="min-w-36 border-b border-r border-border px-3 py-2 text-left font-semibold text-primary"
                        key={variable}
                      >
                        ?{variable}
                      </th>
                ...
                <tbody>
                  {results.rows.map((row, rowIndex) => (
                    <tr
                      className="odd:bg-secondary/40 hover:bg-accent/60"
                      key={rowIndex}
                    >
                      <td className="border-b border-r border-border px-3 py-1.5 text-right text-muted-foreground">
                        {rowIndex + 1}
                      </td>
                      {results.variables.map((variable) => {
                        ...
                        return (
                          <td
                            className="max-w-72 border-b border-r border-border px-3 py-1.5 text-foreground"
                            key={variable}
                            title={value}
                          >
        ...
        <div className="flex h-7 items-center justify-between border-t border-border px-4 text-[9px] text-muted-foreground">
